...
u32 _mali_projob_add_gp_drawcall(struct mali_frame_builder* frame_builder, u64* cmnds, struct bs_shaderpass* projob_shader, mali_bool indexed)
{
	...
	streams = _mali_mem_pool_alloc(pool, mem_size, &streams_mem);
	if (NULL == streams) return 0;

        dummy_streams[ GP_VS_CONF_REG_INP_ADDR( 0 ) ] = _SWAP_ENDIAN_U32_U32(0x0);
	dummy_streams[ GP_VS_CONF_REG_INP_SPEC( 0 ) ] = _SWAP_ENDIAN_U32_U32(GP_VS_VSTREAM_FORMAT_NO_DATA);
	    
	_mali_sys_memcpy_cpu_to_mali_runtime_32( streams, dummy_streams , mem_size , sizeof(u32) ); 
	...
	cmnds[num_cmnds++] = GP_VS_COMMAND_WRITE_INPUT_OUTPUT_CONF_REGS(streams_mem, 0, 1);
	cmnds[num_cmnds++] = GP_VS_COMMAND_WRITE_INPUT_OUTPUT_CONF_REGS(streams_mem, 1, 1);
	...
	cmnds[num_cmnds++] = GP_VS_COMMAND_LOAD_SHADER(
	   ...
	   num_instructions                                /* length */
        ...
        cmnds[num_cmnds++] = GP_VS_COMMAND_WRITE_CONF_REG( ((num_instructions - 1) << 10) | ((num_instructions - 1) << 20), GP_VS_CONF_REG_PROG_PARAM);
