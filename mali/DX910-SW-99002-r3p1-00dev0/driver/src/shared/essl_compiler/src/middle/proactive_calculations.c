...
static memerr move_calculations_to_pilot_shader(pilot_calculations_context *ctx, symbol *pilot_func, run_time_constant_node *rt_node, unsigned num)
{
	...
	}else
	{
		...
		ESSL_CHECK(sym = _essl_new_variable_symbol(ctx->pool, sym_name, inp_type, qual, VAR_KIND_GLOBAL, UNKNOWN_SOURCE_OFFSET));
		...
		ESSL_CHECK(load = _essl_new_load_expression(ctx->pool, VAR_KIND_GLOBAL, address));
		_essl_ensure_compatible_node(load, rt_node->node);
	...
	}else
	{
		...
		ESSL_CHECK(store = _essl_new_store_expression(ctx->pool, VAR_KIND_GLOBAL, address, out));
...
static memerr find_constant_input_calculations_nodes(pilot_calculations_context *ctx)
{
	...
	for (j = ctx->cfg->n_blocks-1 ; j >= 0 ; j--)
	{
		...
		for (cd = block->control_dependent_ops ; cd != NULL ; cd = cd->next)
		{
			...
			{
				if(cd->op->expr.u.load_store.var_kind != VAR_KIND_VERTEX_VARYING &&
					cd->op->expr.u.load_store.var_kind != VAR_KIND_LOCAL)
				{
					continue;
				}
			}
...
memerr _essl_optimise_constant_input_calculations(mempool *pool, typestorage_context *typestor_ctx, translation_unit* tu)
{
	...
	if(_essl_ptrdict_init(&ctx.rt_const_nodes, &ctx.temp_pool) != MEM_OK)
	{
		_essl_mempool_destroy(&ctx.temp_pool);
		return MEM_ERROR;
	}
